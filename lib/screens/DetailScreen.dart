...
class DetailScreenState extends State<DetailScreen> {
  ...
  TextEditingController cashColectedController = TextEditingController();
  ...
  Future<void> savePaymentApi() async {
    ...
    if (currentData!.payment!.paymentType == CASH) {
      req['collected_cash'] = cashColectedController.text.validate();
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            // SizedBox(height: 16),
                            SizedBox(height: 16),
                            totalCount(title: 'Tarifa Negociada', description: '', subTitle: toCurrency(riderModel!.proposedFee!)),
                            SizedBox(height: 16),
                            if (payment != null)
                              totalCount(title: language.tip, description: '', subTitle: toCurrency(payment!.driverTips!)),
                            ...
                            Divider(height: 30, thickness: 1),
                            totalCount(
                                title: 'Total a cobrar', description: '', subTitle: toCurrency(currentData!.payment!.outstanding!)),
                            ...
                            if (currentData!.payment!.paymentType == CASH)
                              Column(
                                children: [
                                  SizedBox(height: 8),
                                  AppTextField(
                                    controller: cashColectedController,
                                    // nextFocus: passFocus,
                                    autoFocus: false,
                                    textFieldType: TextFieldType.PHONE,
                                    keyboardType: TextInputType.number,
                                    errorThisFieldRequired: language.thisFieldRequired,
                                    decoration: inputDecoration(context, label: 'Indique efectivo recolectado*'),
                                  ),
                                ],
                              ),
                          ],
      ...
      bottomNavigationBar: currentData != null
          ? Padding(
              padding: EdgeInsets.all(16),
              child: currentData!.payment!.paymentType == CASH || currentData!.payment!.paymentType == 'mobile-payment'
                  ? AppButtonWidget(
                      ...
                      onTap: () {
                        showConfirmDialogCustom(
                            ...
                            context, onAccept: (v) {
                          bool isCash = currentData!.payment!.paymentType == CASH;

                          if (isCash && cashColectedController.text.isEmpty) {
                            toast('Rellena todos los campos requeridos');
                            // log('CASH_COLECTED Rellena todos los campos requeridos');
                          } else {
                            savePaymentApi();
                            // log('CASH_COLECTED puede continuar');
                          }
