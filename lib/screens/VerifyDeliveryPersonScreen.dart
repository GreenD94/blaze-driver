...
class VerifyDeliveryPersonScreenState extends State<VerifyDeliveryPersonScreen> {
  ...
  getMultipleFile(int? docId, int? isExpire, {int? updateId, DateTime? dateTime}) async {
    bool exceedsMaximumSize = true;
    filePickerResult =
        await FilePicker.platform.pickFiles(type: FileType.custom, allowedExtensions: ['jpg', 'png', 'jpeg', 'pdf']);

    exceedsMaximumSize = (filePickerResult!.files[0].size / 1000) > MAX_UPLOAD_FILE_SIZE;

    if (exceedsMaximumSize) {
      toast('Supera el tamaño máximo de ${(MAX_UPLOAD_FILE_SIZE / 1000).toStringAsFixed(2)} MB.');
    } else {
                                      ...
                                      children: [
                                        Expanded(
                                          child: Text(e.name.validate(),
                                              style: primaryTextStyle(), overflow: TextOverflow.fade, softWrap: false),
                                        ),
                                        SizedBox(width: 4),
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          ...
                          Expanded(
                            child: Text(
                              'El tamaño máximo del archivo que puedes subir es de ${(MAX_UPLOAD_FILE_SIZE / 1000).toStringAsFixed(2)} MB.',
                                ...
                                Row(
                                  children: [
                                    ...
                                    driverDocumentList[index].expireDate != null
                                        ? Expanded(
                                            child:
                                                Text(driverDocumentList[index].expireDate.toString(), style: primaryTextStyle()))
                                        : Expanded(
                                            child: Text(''),
                                          ),
                                    Visibility(
